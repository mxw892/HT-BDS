...
from pyvisa.constants import StatusCode, EventType, EventMechanism     # PyVISA.Constants.StatusCode for discerning error/return codes
from pyvisa.resources import Resource, USBInstrument, GPIBInstrument  # PyVISA.Resources.USBInstrument for type-casting the correct device communication format under VISA
        ...
        while is_waiting:
            try:
                if cmd:
                    try:
                        w_ret = dev.write(cmd)
                        if w_ret != len(cmd + dev.write_termination):
                            # Alert incorrect setting; Setting did not take effect, or expected return is incorrect
                            print("Note: Write returned a mismatch!")
                        cmd = ""
                    except Exception as e:
                        print("WRITE ERR:", e)
                        is_waiting = False
                    ...
                    try:
                        q_ret = dev.query(qry)
                        if expect and (q_ret != expect):
                            # Alert incorrect setting; Setting did not take effect, or expected return is incorrect
                            print("Note: Query returned a mismatch! (This can happen on data fetches)")
                        w_ret = len(qry)
                        qry = ""
                    except Exception as e:
                        print("QUERY ERR:", e)
                ...
                if (e.error_code == StatusCode.error_timeout):
                    if timeout_count > 5:
                        raise Exception("ERR: Timeout! Invalid command, or device is taking too long.")
                    timeout_count += 1
                    print(f"Waiting for LCR response... {timeout_count}/5")
            except Exception as e:
